...
jQuery(document).ready(function($) {
	...
	tinymce.PluginManager.add('hm_bbpuip_image', function(editor) {
		...
		function showDialog(imageList) {
			...
			if (editor.id == 'bbp_topic_content') {
				// The user is editing a topic
				if (typeof hm_bbpuip_count_limit != 'undefined' && hm_bbpuip_count_limit) {
					var maxFiles = hm_bbpuip_count_limit - hm_bbpuip_get_uploaded_image_count();
					if (maxFiles <= 0) {
				...
				if (typeof hm_bbpuip_count_limit_reply != 'undefined' && hm_bbpuip_count_limit_reply) {
					var maxFiles = hm_bbpuip_count_limit_reply - hm_bbpuip_get_uploaded_image_count();
					if (maxFiles <= 0) {
			...
			function onSubmitForm(imgUrl, imgAlt) {
			...
			*/
			...
			$('.bbpuip_upload_dialog:first').clone().dialog({
				'resizable': false,
				'width': Math.min(Math.max($(window).width() - 50, 200), 500),
				'modal': true,
				'create': function() {
				},
				'open': function() {
					var uploadUrl = location.href;
					var uploadUrlAnchorPos = uploadUrl.indexOf('#');
					if (uploadUrlAnchorPos != -1) {
						uploadUrl = uploadUrl.substr(0, uploadUrlAnchorPos);
					}
					uploadUrl += (uploadUrl.indexOf('?') == -1 ? '?' : '&') + 'hm_bbpuip_do_upload=1';
					$(this).find('.bbpuip_drop_area').dropzone({
						url: uploadUrl,
						paramName: 'hm_bbpuip_file',
						createImageThumbnails: false,
						previewTemplate: '<span></span>',
						clickable: $(this).find('.bbpuip_drop_area_container *').get(),
						'maxFiles': maxFiles,
						init: function() {
							this.on('success', function(obj, response) {
								response = response.trim();
								if (response.substr(0, 4) == 'http') {
									filenameDotPos = obj.name.lastIndexOf('.');
									var imgAlt = (filenameDotPos == -1 ? obj.name : obj.name.substr(0, filenameDotPos));
									onSubmitForm(response, imgAlt);
								} else if (response == 'Px_Size_Error') {
							...
							this.on('totaluploadprogress', function(progress) {
								$(this.element).find('h3').text(hm_bbpuip_strings.uploading);
								$(this.element).find('p').text(Math.round(progress) + hm_bbpuip_strings.percent_complete);
							}),
							this.on('queuecomplete', function() {
								$(this.element).closest('.bbpuip_upload_dialog').dialog('destroy');
							}),
